...
class TestUpload(TestCase):
    ...
    def test_work_data(self):
        ...
        for sheet_name in mandatory_sheets.keys():
            ...
            if sheet_name == 'Work':
                ws.append([1,"test","J",1660,1,1,1660,1,2,1,1,1,1,"test","newton",15257,"test",1,1,"test","Wren",
        ...
        cuef = CofkUploadExcelFile(self.new_upload, tf.name)
        print(cuef.errors)

        self.assertEqual(cuef.errors['work']['total'], 3)
        self.assertEqual(cuef.errors['work']['errors'][0]['errors'][0], msg)
    ...
    def test_people_data(self):
        ...
        for sheet_name in mandatory_sheets.keys():
            ...
            if sheet_name == 'Work':
                ws.append([1,"test","J",1660,1,1,1660,1,2,1,1,1,1,"test","newton",15257,"test",1,1,"test","Wren",
                           ...
                           "test","fra;eng",'','','','','','',"test","test","test","Baskerville",885,"test",
                           "test","EMLO","http://emlo.bodleian.ox.ac.uk/","Early Modern Letters Online test"])
            elif sheet_name == 'People':
                ws.append(["Baskerville", 885])
                ws.append(["newton", 15257])
                ws.append(["Wren", 22859])
        ...
        msg = 'Person with the id 15257 is not in the Union catalogue.'
        #self.assertRaisesRegex(CofkExcelFileError, msg, CofkUploadExcelFile,
        #                       self.new_upload, tf.name)
