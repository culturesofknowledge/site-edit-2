...
from clonefinder.services import clonefinder_schedule, clonefinder
from clonefinder.services.clonefinder import IdsCluster
...
def load_cluster_results(_create_id_records_dict, cluster_factory, model_name):
    record = ClonefinderRequest.objects.filter(model_name=model_name).first()
    clusters = []
    last_update_at = None
...
@permission_required(constant.PM_CLONEFINDER_WORK)
def home(request):
    return render(request, 'clonefinder/clonefinder_basic.html')
...
def similar_work(request):
    ...
    return render_cluster_results(request, clusters, 'clonefinder/clonefinder_work.html',
                                  merge_page_url=(reverse('work:merge')),
                                  is_running=clonefinder_schedule.work_status_handler.is_pending_or_running(),
                                  last_update_at=last_update_at)


@permission_required(constant.PM_CLONEFINDER_LOCATION)
def similar_location(request):
    ...
    return render_cluster_results(request, clusters, 'clonefinder/clonefinder_location.html',
                                  merge_page_url=(reverse('location:merge')),
                                  is_running=clonefinder_schedule.location_status_handler.is_pending_or_running(),
                                  last_update_at=last_update_at)


@permission_required(constant.PM_CLONEFINDER_PERSON)
def similar_person(request):
    ...
    return render_cluster_results(request, clusters, 'clonefinder/clonefinder_person.html',
                                  merge_page_url=(reverse('person:merge')),
                                  is_running=clonefinder_schedule.person_status_handler.is_pending_or_running(),
                                  last_update_at=last_update_at)


@permission_required(constant.PM_CLONEFINDER_INST)
def similar_inst(request):
    ...
    return render_cluster_results(request, clusters, 'clonefinder/clonefinder_inst.html',
                                  merge_page_url=(reverse('institution:merge')),
                                  is_running=clonefinder_schedule.inst_status_handler.is_pending_or_running(),
                                  last_update_at=last_update_at)
...
def trigger_work_clustering(request):
    ...
    clonefinder.trigger_clustering(CofkUnionWork.__name__, queryset,
                                 clonefinder_schedule.work_status_handler,
                                 username=request.user.username)
    return redirect('clonefinder:work')
...
def trigger_location_clustering(request):
    ...
    clonefinder.trigger_clustering(CofkUnionLocation.__name__, queryset,
                                 clonefinder_schedule.location_status_handler,
                                 username=request.user.username)
    return redirect('clonefinder:location')
...
def trigger_person_clustering(request):
    ...
    clonefinder.trigger_clustering(CofkUnionPerson.__name__, queryset,
                                 clonefinder_schedule.person_status_handler,
                                 username=request.user.username)
    return redirect('clonefinder:person')
...
def trigger_inst_clustering(request):
    ...
    clonefinder.trigger_clustering(CofkUnionInstitution.__name__, queryset,
                                 clonefinder_schedule.inst_status_handler,
                                 username=request.user.username)
    return redirect('clonefinder:inst')
